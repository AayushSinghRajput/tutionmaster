...
const { chat } = require("./backend/services/ai/agent");
const dotenv = require("dotenv");
dotenv.config({ path: "./backend/.env" });

async function runTests() {
  console.log("Connecting to DB...");
  await mongoose.connect(process.env.MONGO_URI || "mongodb://localhost:27017/tuitionmaster");
  ...
  const history = [
    ...
    { role: "assistant", content: res1.message } // not passing results to history to see if it relies on text or tool context? Wait, the UI passes standard message history.
  ];
