...
const FormProgressHeader = ({ isEdit, currentStep, steps }) => (
  <div className="bg-gradient-to-r from-blue-700 via-blue-600 to-indigo-600 px-4 py-5 sm:px-6 sm:py-6 lg:px-10 lg:py-8">
    {/* Title row */}
    <div className="flex flex-wrap items-center justify-between gap-3 mb-6 sm:mb-8">
      <div className="flex items-center space-x-3 sm:space-x-4 min-w-0">
        ...
        <div className="min-w-0">
          <h1 className="text-lg sm:text-3xl font-bold text-white truncate">
            {isEdit ? "Update Teacher Profile" : "Create Teacher Profile"}
          </h1>
          <p className="hidden sm:block text-blue-100 text-lg mt-1">
            {isEdit
          ...
          <div key={step.number} className="flex items-center flex-1 z-10">
            ...
            {index < steps.length - 1 && (
              <div className="flex-1 mx-2 sm:mx-3 lg:mx-4 relative">
                ...
                ></div>
                {isCompleted && (
                  <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-green-400 absolute -top-1.5 sm:-top-2 left-1/2 transform -translate-x-1/2 bg-blue-600 rounded-full" />
                )}
              </div>
            )}
          </div>
