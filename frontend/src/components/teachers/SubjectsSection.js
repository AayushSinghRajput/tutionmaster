import { BookOpen } from 'lucide-react';

const SubjectsSection = ({ subjects }) => {
  return (
    <section className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
      <h2 className="font-serif text-xl sm:text-2xl font-semibold text-gray-900 mb-4">Subjects I Teach</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
        {subjects.map((subject, index) => (
          <div key={index} className="flex items-center space-x-3 p-3 bg-brand-50 rounded-lg min-w-0">
            <BookOpen className="w-5 h-5 text-brand-600 flex-shrink-0" />
            <span className="text-gray-700 font-medium truncate">{subject}</span>
          </div>
        ))}
      </div>
    </section>
  );
};

export default SubjectsSection;